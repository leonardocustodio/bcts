...
> **Note:** This package is now a compatibility layer over
> [`@blockchaincommons/dcbor`](https://github.com/BlockchainCommons/bc-dcbor-ts),
> the canonical dCBOR implementation for TypeScript. The `@bcts/dcbor` API is
> unchanged, but all wire-format logic (encoding, decoding, canonical map
> ordering, diagnostics, traversal) is delegated to the canonical package.
> New code should consider depending on `@blockchaincommons/dcbor` directly;
> see its [migration guide](https://github.com/BlockchainCommons/bc-dcbor-ts/blob/master/MIGRATION.md).
